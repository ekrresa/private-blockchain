...
class Blockchain {
  ...
  _addBlock(block) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (chainErrors.length > 0) {
        return reject("Chain is invalid");
      }

      try {
        const lastBlock = self.chain[self.chain.length - 1];

        block.previousBlockHash = self.height === -1 ? null : lastBlock.hash;
        block.height = self.height + 1;
        block.time = new Date().getTime().toString().slice(0, -3);

        const { body, height, previousBlockHash, time } = block;

        block.hash = SHA256(
          JSON.stringify({ body, height, previousBlockHash, time })
        ).toString();
  ...
  submitStar(address, message, signature, star) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const differenceInMinutes = Math.abs(
        Math.round(differenceInMilliseconds / 60000)
      );
      ...
      const block = await this._addBlock(new Block({ owner: address, star }));
  ...
  getStarsByWalletAddress(address) {
    ...
    let stars = [];
    return new Promise(async (resolve, reject) => {
      const blockDataMap = await Promise.all(
        self.chain.map(async (block) => await block.getBData())
      );

      console.log({ blockDataMap });
